...
namespace DaydreamElements.Tunneling {
  ...
  public class FirstPersonTunnelingLocomotion : MonoBehaviour {
    ...
    [Tooltip("GameObject Plane that controls elevation.")]
    ...
    private Vector2? clickTouch = null;
    ...
    private const float SLOP_HORIZONTAL = 0.15f;
        ...
        public BaseVignetteController VignetteController
        {
            get
            {
    ...
    protected virtual void Update() {
      ...
      if (GvrControllerInput.ClickButtonDown) {
        clickTouch = GvrControllerInput.TouchPos;
      ...
      if (isPlatformMoving) {
        PlatformMove();
        plane.transform.position = target;
        clickTouch = null;
      }
    }

    protected virtual void PlatformMove() {
      Vector3 startPos = plane.transform.position;
      if (clickTouch.Value.x > 0.3f && clickTouch.Value.x <= 0.6f && clickTouch.Value.y > 0f && clickTouch.Value.y <= 0.3f) {
        target = new Vector3(startPos.x, startPos.y + 5, startPos.z);
      }
      else if (clickTouch.Value.x > 0.3f && clickTouch.Value.x <= 0.6f && clickTouch.Value.y > 0.7f && clickTouch.Value.y <= 1f) {
        target = new Vector3(startPos.x, startPos.y - 5, startPos.z);
      }
